...
export function normalizeCardWidth(value: number | undefined | null): number {
  return Math.min(CARD_WIDTH_MAX, Math.max(CARD_WIDTH_MIN, Math.round(value ?? CARD_WIDTH_DEFAULT)));
}

export function normalizeCardHeight(value: number | undefined | null): number {
  return Math.min(CARD_HEIGHT_MAX, Math.max(CARD_HEIGHT_MIN, Math.round(value ?? CARD_HEIGHT_DEFAULT)));
}
